      ...
      return {
        ...
        getNum: function(key, data){
          ...
          if (items.length === keys.length) {
        	  a['count']=data[key] ? data[key].count : 0;
        	  a['price']=data[key] ? data[key].price : 0;
            return a;
            //
          }
          ...
          for (i = 0; i < keys.length; i++) {
            ...
            if (j < keys[i].length && items.length > 0) {
              ...
              for (m = 0; m < keys[i].length; m++) {
            	  //
            	  var a=this.getNum(n.concat(keys[i][m], items).join(skuConfig.splitStr), data);
            	  result += a['count'];
                //
              }
              break;
    ...
    .directive('uiSku', ['$log', 'skuConfig', 'utilService', function($log, skuConfig, utilService){
      return{
        ...
        link: function(scope, element, attrs, ctrls, transclude) {
          ...
          scope.$watch('skuData',function(a){
          ...
          scope.checkItem = function(currentKey){
            ...
            angular.forEach(scope.keys, function(array, i0){
              ...
              angular.forEach(array, function(key, i1) {
                ...
                //
                var a=utilService.getNum(check.join(skuConfig.splitStr), scope.skuData);
                if($.type(a)=='number'){//数字
                	keyMap[key].disabled = a> 0? false: true;
                }else if($.type(a)=='array'){
                	keyMap[key].disabled = a['count']> 0? false: true;
           	    }
              });
            });
          };
